...
const App: React.FC = () => {
  ...
  const tipoRecuperacionSeguro = searchParams.get('type');
  const tokenHashRecuperacion = searchParams.get('token_hash');
  const confirmationUrlRecuperacion = searchParams.get('confirmation_url');

  if (tipoRecuperacionSeguro === 'recovery' && (tokenHashRecuperacion || confirmationUrlRecuperacion)) {
    return (
      <PantallaAccesoRecuperacionContrasena
        tokenHash={tokenHashRecuperacion}
        confirmationUrl={confirmationUrlRecuperacion}
...
const InvitationProcessor: React.FC = () => {
  ...
  const aceptarInvitacion = async () => {
    ...
    try {
      ...
      console.log('[INVITATION] Upsert payload:', JSON.stringify(upsertPayload));
      const { error: upsertError } = await supabase
        .from('miembros_espacio')
        .upsert(upsertPayload, { onConflict: 'espacio_id,usuario_id' });
