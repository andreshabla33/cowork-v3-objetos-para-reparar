      ...
      <div className="bg-[#181825] border border-white/5 rounded-2xl lg:rounded-xl p-6 lg:p-5 md:p-4 w-full max-w-sm lg:max-w-xs shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 lg:space-y-3">
          <div>
            ...
            <input type="text" value={nombre} onChange={(e) => setNombre(e.target.value)} placeholder={t('chat.namePlaceholder', currentLang)} className="w-full bg-[#1e1e2e] border border-white/5 rounded-xl lg:rounded-lg px-4 lg:px-3 py-3 lg:py-2.5 text-sm lg:text-xs text-white focus:ring-1 focus:ring-blue-700 outline-none font-bold" autoFocus />
          </div>
          <div className="grid grid-cols-2 gap-2.5 lg:gap-2">
            <button type="button" onClick={() => { setTipo('publico'); setContrasena(''); }} className={`p-3 lg:p-2.5 rounded-xl lg:rounded-lg border-2 flex flex-col items-center gap-1 ${tipo === 'publico' ? 'border-blue-700 bg-blue-700/10 text-white' : 'border-white/5 text-zinc-500'}`}><span className="text-lg lg:text-base">#</span><span className="text-[8px] lg:text-[7px] font-black uppercase tracking-widest">{t('chat.public', currentLang)}</span></button>
            <button type="button" onClick={() => setTipo('privado')} className={`p-3 lg:p-2.5 rounded-xl lg:rounded-lg border-2 flex flex-col items-center gap-1 ${tipo === 'privado' ? 'border-blue-700 bg-blue-700/10 text-white' : 'border-white/5 text-zinc-500'}`}><span className="text-lg lg:text-base">🔒</span><span className="text-[8px] lg:text-[7px] font-black uppercase tracking-widest">{t('chat.private', currentLang)}</span></button>
          </div>
          {tipo === 'privado' && (
            <div>
              ...
              <input type="password" value={contrasena} onChange={(e) => setContrasena(e.target.value)} placeholder="Contrase\u00f1a para acceder al canal" className="w-full bg-[#1e1e2e] border border-white/5 rounded-xl lg:rounded-lg px-4 lg:px-3 py-3 lg:py-2.5 text-sm lg:text-xs text-white focus:ring-1 focus:ring-blue-700 outline-none font-bold" />
            </div>
          )}
          <div className="flex gap-2.5 lg:gap-2">
            ...
            <button type="submit" disabled={!nombre.trim() || creando || (tipo === 'privado' && !contrasena.trim())} className="flex-1 bg-blue-700 text-white py-3 lg:py-2.5 rounded-xl lg:rounded-lg font-black uppercase tracking-widest text-[9px] lg:text-[8px] disabled:opacity-20 transition-all">{t('button.create', currentLang)}</button>
          </div>
        </form>
      </div>
    </div>
