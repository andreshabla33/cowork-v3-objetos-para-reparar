          ...
          <div className="relative backdrop-blur-xl bg-white/[0.03] border border-white/[0.08] rounded-[36px] lg:rounded-[28px] p-6 lg:p-5">
            ...
            <div className="relative group mx-auto w-14 h-14 lg:w-12 lg:h-12 mb-4">
              <div className={`absolute -inset-2 rounded-xl blur-lg opacity-40 ${
                esAdmin ? 'bg-gradient-to-r from-blue-600 to-cyan-500' : 'bg-gradient-to-r from-cyan-500 to-blue-600'
              }`} />
              <div className={`relative w-14 h-14 lg:w-12 lg:h-12 rounded-xl flex items-center justify-center ${
                esAdmin ? 'bg-gradient-to-br from-blue-600 via-sky-500 to-cyan-500' : 'bg-gradient-to-br from-cyan-500 via-blue-600 to-cyan-500'
              }`}>
            ...
            <p className="text-zinc-500 text-xs lg:text-[10px] mb-4">
              Te uniste a <span className="text-sky-400 font-medium">{state.espacioNombre}</span>
              {state.invitadorNombre && (
                <span> por invitación de <span className="text-sky-400 font-medium">{state.invitadorNombre}</span></span>
              )}
            </p>

            {esAdmin ? (
              <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-4 lg:p-3 text-left mb-4">
                <h3 className="text-sky-400 font-black text-[10px] uppercase tracking-widest mb-2">Como administrador podrás:</h3>
                <ul className="space-y-1.5 text-[11px] lg:text-[10px] text-zinc-400">
                  <li className="flex items-center gap-2"><span className="text-sky-400">✓</span> Invitar y gestionar miembros del equipo</li>
                  <li className="flex items-center gap-2"><span className="text-sky-400">✓</span> Configurar métricas de análisis conductual</li>
                  <li className="flex items-center gap-2"><span className="text-sky-400">✓</span> Administrar departamentos y roles</li>
                  <li className="flex items-center gap-2"><span className="text-sky-400">✓</span> Acceder a configuraciones avanzadas del espacio</li>
                </ul>
              </div>
            ) : (
            ...
            <button
              ...
              className="relative w-full group overflow-hidden bg-gradient-to-r from-blue-600 via-sky-500 to-cyan-500 text-white py-3 lg:py-2.5 rounded-xl font-black text-xs lg:text-[10px] uppercase tracking-[0.15em] transition-all shadow-2xl shadow-blue-600/30 active:scale-[0.98]"
            >
    ...
    <div className="fixed inset-0 bg-[#050508] flex items-center justify-center p-4">
      ...
      <div className="w-full max-w-xl lg:max-w-lg relative z-10">
        <div className="text-center mb-6 lg:mb-5">
          <button
            onClick={handleBack}
            className="mb-3 text-zinc-500 hover:text-sky-400 flex items-center gap-1.5 mx-auto text-[10px] font-bold uppercase tracking-widest transition-colors"
          >
            ← Volver a selección de cargo
          </button>
          <div className="inline-flex items-center gap-2 px-3 py-1 bg-gradient-to-r from-blue-600/20 to-cyan-500/20 border border-blue-500/30 rounded-full text-sky-400 text-[9px] font-bold uppercase tracking-wider mb-3">
            Paso 2 de 3
          </div>
          <h1 className="text-2xl lg:text-xl font-black text-transparent bg-clip-text bg-gradient-to-r from-white via-sky-200 to-white mb-1">¿A qué departamento perteneces?</h1>
          <p className="text-zinc-500 text-xs lg:text-[10px]">
            Selecciona tu departamento en <span className="text-sky-400 font-medium">{state.espacioNombre}</span>
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 lg:gap-2.5">
          {state.departamentos.map((dept) => (
            <button
              ...
              disabled={saving}
              className="group p-4 lg:p-3 rounded-xl border border-white/[0.08] bg-white/[0.02] backdrop-blur-xl hover:border-blue-500/40 transition-all duration-200 text-left disabled:opacity-50"
            >
              <div
              ...
              <h3 className="text-sm lg:text-xs font-bold text-white group-hover:text-sky-400 transition-colors">
                {dept.nombre}
              </h3>
              <div
