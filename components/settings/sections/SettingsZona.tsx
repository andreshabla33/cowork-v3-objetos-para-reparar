...
export const SettingsZona: React.FC<SettingsZonaProps> = ({ workspaceId, isAdmin, onCloseModal }) => {
  ...
  const [zonaPlantillaActiva, setZonaPlantillaActiva] = useState<ZonaEmpresa | null>(null);
  const [plantillaZonaSeleccionada, setPlantillaZonaSeleccionada] = useState<PlantillaZonaId>('cubiculo');

  const plantillaZonaSeleccionadaData = useMemo(() => {
    return obtenerPlantillaZona(plantillaZonaSeleccionada);
  }, [plantillaZonaSeleccionada]);
  ...
  const obtenerPlantillaAplicadaZona = useCallback((zona: ZonaEmpresa) => {
    const configuracion = normalizarConfiguracionZonaEmpresa(zona.configuracion);
    return obtenerPlantillaZona(configuracion.plantilla_zona?.id);
  }, []);

  const handleAbrirPlantillasZona = useCallback((zona: ZonaEmpresa) => {
    const configuracion = normalizarConfiguracionZonaEmpresa(zona.configuracion);
    const plantillaActual = obtenerPlantillaZona(configuracion.plantilla_zona?.id);
    setPlantillaZonaSeleccionada(plantillaActual?.id ?? 'cubiculo');
    setZonaPlantillaActiva(zona);
  }, []);

  const handleIniciarColocacionPlantillaZona = useCallback((zona: ZonaEmpresa, plantillaId: PlantillaZonaId) => {
    const plantilla = obtenerPlantillaZona(plantillaId);
    if (!plantilla) {
      mostrarMensaje('error', 'La plantilla seleccionada no es válida.');
      return;
    }

    const configuracion = normalizarConfiguracionZonaEmpresa(zona.configuracion);
    setPlantillaZonaSeleccionada(plantillaId);
    setIsEditMode(false);
    setPlantillaZonaEnColocacion({
      zonaId: zona.id,
      workspaceId,
      plantillaId,
      nombrePlantilla: plantilla.nombre,
      nombreZona: zona.nombre_zona || 'Zona sin nombre',
      posicionX: configuracion.plantilla_zona?.centro_x ?? Number(zona.posicion_x) / 16,
      posicionZ: configuracion.plantilla_zona?.centro_z ?? Number(zona.posicion_y) / 16,
      anchoMetros: plantilla.ancho_minimo_metros,
      altoMetros: plantilla.alto_minimo_metros,
    });
    setZonaPlantillaActiva(null);
  ...
  // === Estado de progreso para plantilla completa ===
  const [progresoPlantilla, setProgresoPlantilla] = useState('');
  const [confirmarPlantilla, setConfirmarPlantilla] = useState(false);

  // === Generador dinámico: aplicar ===
  const handleAplicarPlantillaCompleta = async () => {
    if (!plantillaCompletaSeleccionada || !usuarioId || !empresaUsuarioId) return;
    ...
    try {
      ...
      const useCase = new AplicarPlantillaEspacioCompletaUseCase(
        repositorio,
        new AplicarPlantillaZonaUseCase(new RepositorioPlantillaZonaSupabase(), new InyectorPlantillaZona())
      );
      ...
      const res = await useCase.execute({
        ...
        userId: usuarioId,
        plantillaCompletaId: plantillaCompletaSeleccionada,
        onProgreso: ({ paso, total, zona }) => {
          setProgresoPlantilla(`Generando zona ${paso}/${total}: ${zona}`);
        },
      });

      if (res.exito) {
        const msg = res.errores.length > 0
          ? `Plantilla aplicada (${res.zonasGeneradas}/${res.zonasTotal} zonas, ${res.errores.length} errores). Recargando...`
          : `Plantilla aplicada correctamente (${res.zonasGeneradas} zonas). Recargando...`;
        mostrarMensaje('exito', msg);
      } else {
        mostrarMensaje('error', `Fallo total: ${res.errores.map(e => e.zona).join(', ')}`);
      }
    } catch (error: any) {
      console.error(error);
      mostrarMensaje('error', error.message || 'Error al aplicar plantilla');
    } finally {
      setAplicandoPlantillaCompleta(false);
      setProgresoPlantilla('');
    }
  };
  ...
  return (
    <div className="space-y-10">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        {!mostrarFormulario && !mostrarGenerador && !mostrarGeneradorCompleto && (
      ...
      {mostrarGeneradorCompleto && (
        <div className="p-5 bg-gradient-to-br from-zinc-800/80 to-zinc-900/80 border border-violet-500/20 rounded-2xl shadow-xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
            {PLANTILLAS_ESPACIO_COMPLETAS.map(plantilla => (
              <button
                key={plantilla.id}
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 text-[10px] text-red-400">
              <Trash2 className="w-3 h-3" /> Aplicar borrará todas las zonas y objetos actuales
            </div>
            <div className="flex items-center gap-2">
              ...
              {!confirmarPlantilla ? (
                ...
                <button
                  onClick={handleAplicarPlantillaCompleta}
                  disabled={aplicandoPlantillaCompleta}
                  className="flex items-center gap-2 px-5 py-2 bg-gradient-to-r from-red-600 to-red-500 hover:from-red-500 hover:to-red-400 text-white rounded-lg text-sm font-medium transition-all shadow-lg shadow-red-500/20 disabled:opacity-50 animate-pulse"
                >
                  {aplicandoPlantillaCompleta ? (
                    <><RefreshCw className="w-4 h-4 animate-spin" /> Generando...</>
                  ) : (
                    <><Trash2 className="w-4 h-4" /> Confirmar: Borrar y Aplicar</>
                  )}
                </button>
              )}
            </div>
                        ...
                        <div className="rounded-xl border border-white/5 bg-black/30 p-2 mb-3">
                          <svg viewBox="0 0 100 100" className="w-full aspect-square rounded-lg overflow-hidden">
                            ...
                            {plantilla.preview.bloques.map((bloque, indice) => (
                              <rect
                                key={`${plantilla.id}-${indice}`}
                                x={bloque.x}
                                y={bloque.y}
                                width={bloque.ancho}
                                height={bloque.alto}
                                rx={bloque.redondeado ? 6 : 0}
                                fill={bloque.color}
                              />
                            ))}
                          </svg>
