...
const log = logger.child('virtual-space');
...
interface MinimapProps {
 currentUser: User;
 users: User[];
 workspace: { width: number; height: number } | null;
}

interface AvatarPixelConfig {
 skinColor: string;
 hairColor: string;
 clothingColor: string;
}

interface CoworkScene extends Phaser.Scene {
 ...
 remotePlayers: Map<string, Phaser.GameObjects.Container>;
 cursors: Phaser.Types.Input.Keyboard.CursorKeys;
 wasd: Record<string, Phaser.Input.Keyboard.Key>;
 lastSyncTime: number;
 isPanning: boolean;
 floatEmoji: (emoji: string) => void;
 generatePixelAvatar: (key: string, config: AvatarPixelConfig) => void;
 createRemotePlayer: (u: User, tex: string) => void;
}
...
// --- Minimap Component ---
const Minimap: React.FC<MinimapProps> = ({ currentUser, users, workspace }) => {
 ...
 return (
 <div className="absolute bottom-6 left-6 w-[140px] h-[140px] bg-[#0B2240]/35 backdrop-blur-[10px] backdrop-blur-xl rounded-2xl border border-[rgba(46,150,245,0.14)] overflow-hidden pointer-events-none shadow-2xl z-20">
...
// --- VideoHUD Component ---
const VideoHUD = React.memo((props: VideoHUDProps) => {
 const {
 ...
 localVideoRef.current.play().catch((err: unknown) => {
 const message = err instanceof Error ? err.message : String(err);
 log.warn('Auto-play error on local video', { error: message });
 });
 }
 ...
 expandedVideoRef.current.play().catch((err: unknown) => {
 const message = err instanceof Error ? err.message : String(err);
 log.warn('Auto-play error on expanded video', { error: message });
 });
 }
 ...
 const accentColor = theme === 'arcade' ? 'bg-[#00ff41] text-black' : 'bg-indigo-600 text-[#0B2240]';
 ...
 {isLocal && (
 <>
 <button onClick={(e) => { e.stopPropagation(); onToggleMic(); }} className={`w-8 h-8 rounded-full flex items-center justify-center backdrop-blur-md border border-[rgba(46,150,245,0.14)] transition-all ${micOn ? 'bg-white/20 text-[#0B2240] hover:bg-white/40' : 'bg-red-500 text-[#0B2240] shadow-lg'}`}><IconMic on={micOn}/></button>
 <button onClick={(e) => { e.stopPropagation(); onToggleCam(); }} className={`w-8 h-8 rounded-full flex items-center justify-center backdrop-blur-md border border-[rgba(46,150,245,0.14)] transition-all ${camOn ? 'bg-white/20 text-[#0B2240] hover:bg-white/40' : 'bg-red-500 text-[#0B2240] shadow-lg'}`}><IconCam on={camOn}/></button>
 <button onClick={(e) => { e.stopPropagation(); onToggleShare(); }} className={`w-8 h-8 rounded-full flex items-center justify-center backdrop-blur-md border border-[rgba(46,150,245,0.14)] transition-all ${sharingOn ? accentColor : 'bg-white/20 text-[#0B2240] hover:bg-white/40'}`}><IconScreen on={sharingOn}/></button>
 <button onClick={(e) => { e.stopPropagation(); onTogglePrivacy(); }} className={`w-8 h-8 rounded-full flex items-center justify-center backdrop-blur-md border border-[rgba(46,150,245,0.14)] transition-all ${isPrivate ? 'bg-[#2E96F5] text-[#0B2240] shadow-lg' : 'bg-white/20 text-[#0B2240] hover:bg-white/40'}`}><IconPrivacy on={isPrivate}/></button>
 <div className="relative">
 <button onClick={(e) => { e.stopPropagation(); setShowEmojiPicker(showEmojiPicker === targetId ? null : targetId); }} className="w-8 h-8 rounded-full flex items-center justify-center bg-white/20 backdrop-blur-md border border-[rgba(46,150,245,0.14)] text-[#0B2240] hover:bg-white/40 transition-all"><IconReaction /></button>
 {showEmojiPicker === targetId && (
 <div className="absolute bottom-10 left-1/2 -translate-x-1/2 bg-[#0B2240]/35 backdrop-blur-[10px] backdrop-blur-xl p-1.5 rounded-2xl border border-[rgba(46,150,245,0.14)] flex gap-1 shadow-2xl animate-in slide-in-from-bottom-2">
 {emojis.map(e => (
 ...
 className="w-8 h-8 rounded-full bg-indigo-600 backdrop-blur-md border border-[rgba(46,150,245,0.14)] flex items-center justify-center text-[#0B2240] hover:bg-indigo-500 transition-all shadow-xl"
 >
 ...
 <div className={`fixed inset-0 bg-[#0B2240]/35 backdrop-blur-[10px] backdrop-blur-md transition-opacity duration-500 pointer-events-none ${expandedId ? 'opacity-100' : 'opacity-0'}`} />
 ...
 <div className={`flex-1 relative bg-black rounded-[40px] overflow-hidden border ${theme === 'arcade' ? 'border-[#00ff41]' : 'border-[rgba(46,150,245,0.14)]'} shadow-2xl group/expanded`}>
 ...
 <div 
 className="w-full h-full flex items-center justify-center bg-white/60 overflow-hidden pointer-events-none select-none"
 style={{ transform: `translate(${panOffset.x}px, ${panOffset.y}px) scale(${zoomLevel})` }}
 >
 ...
 <p className="text-xl font-bold uppercase tracking-widest text-[#0B2240]">{expandedUser?.name}</p>
 </div>
 </div>
 )}
 </div>
 
 <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex items-center gap-3 p-3 rounded-[32px] bg-[#0B2240]/35 backdrop-blur-[10px] backdrop-blur-2xl border border-[rgba(46,150,245,0.14)] shadow-2xl transition-opacity opacity-0 group-hover/expanded:opacity-100">
 {expandedId === 'local' && (
 <>
 <button onClick={onToggleMic} className={`w-12 h-12 rounded-full flex items-center justify-center transition-all ${micOn ? 'bg-[rgba(46,150,245,0.08)] text-[#0B2240] hover:bg-white/20' : 'bg-red-500 text-[#0B2240]'}`}><IconMic on={micOn} /></button>
 <button onClick={onToggleCam} className={`w-12 h-12 rounded-full flex items-center justify-center transition-all ${camOn ? 'bg-[rgba(46,150,245,0.08)] text-[#0B2240] hover:bg-white/20' : 'bg-red-500 text-[#0B2240]'}`}><IconCam on={camOn} /></button>
 <button onClick={onToggleShare} className={`w-12 h-12 rounded-full flex items-center justify-center transition-all ${sharingOn ? accentColor : 'bg-[rgba(46,150,245,0.08)] text-[#0B2240] hover:bg-white/20'}`}><IconScreen on={sharingOn}/></button>
 <button onClick={onTogglePrivacy} className={`w-12 h-12 rounded-full flex items-center justify-center transition-all ${isPrivate ? 'bg-[#2E96F5] text-[#0B2240]' : 'bg-[rgba(46,150,245,0.08)] text-[#0B2240] hover:bg-white/20'}`}><IconPrivacy on={isPrivate}/></button>
 </>
 )}
 <button 
 onClick={() => { setZoomLevel(prev => prev >= 3 ? 1 : prev + 0.5); setPanOffset({x:0, y:0}); }} 
 className="w-12 h-12 rounded-full bg-[rgba(46,150,245,0.08)] backdrop-blur-md text-[#0B2240] flex items-center justify-center hover:bg-white/20 transition-all border border-[rgba(46,150,245,0.14)]"
 title="Ajustar Zoom"
 >
 <IconZoom />
 </button>
 ...
 className="w-12 h-12 rounded-full bg-indigo-600 backdrop-blur-md text-[#0B2240] flex items-center justify-center hover:bg-indigo-500 transition-all border border-[rgba(46,150,245,0.14)] shadow-lg"
 title="Minimizar"
 >
 <IconExpand on={true} />
 </button>
 ...
 {(expandedId !== 'local') && (
 <div className={`relative bg-black rounded-[28px] overflow-hidden border ${theme === 'arcade' ? 'border-[#00ff41]' : 'border-[rgba(46,150,245,0.14)]'} shadow-2xl group transition-all shrink-0 ${expandedId ? 'w-full aspect-video' : 'w-48 h-32 md:w-60 md:h-40'}`}>
 ...
 <div className={`w-14 h-14 rounded-full border border-indigo-500/30 flex items-center justify-center text-indigo-400 font-black text-2xl bg-[#0B2240]/35 backdrop-blur-[10px]`}>{userName.charAt(0)}</div>
 </div>
 )}
 ...
 {usersInCall.map((u: User) => {
 const remoteStream = remoteStreams?.get(u.id);
 return (expandedId !== u.id) && (
 <div key={u.id} className={`relative bg-white/60 rounded-[28px] overflow-hidden border ${theme === 'arcade' ? 'border-[#00ff41]' : 'border-[rgba(46,150,245,0.14)]'} shadow-2xl group transition-all shrink-0 ${expandedId ? 'w-full aspect-video' : 'w-48 h-32 md:w-60 md:h-40'}`}>
 {remoteStream ? (
 <video 
 ...
 <div className={`w-14 h-14 rounded-full border border-[rgba(46,150,245,0.14)] flex items-center justify-center text-[#0B2240] font-black text-2xl bg-white/70`}>{u.name.charAt(0)}</div>
 </div>
 </>
 )}
 <BubbleControls isLocal={false} targetId={u.id} />
 <div className="absolute top-3 left-3 flex items-center gap-2 bg-[#0B2240]/35 backdrop-blur-[10px] backdrop-blur-md px-3 py-1.5 rounded-xl border border-[rgba(46,150,245,0.14)] shadow-lg">
 <div className={`w-2 h-2 rounded-full ${u.isMicOn ? 'bg-green-500 animate-pulse' : 'bg-red-500'}`} />
 <span className="text-[9px] font-black uppercase tracking-widest text-[#0B2240] truncate max-w-[80px]">{u.name}</span>
 </div>
 </div>
 );
 })}
...
export const VirtualSpace: React.FC = () => {
 ...
 // WebRTC Signaling - sendWebRTCMessage will be provided by the hook
 const sendWebRTCMessageRef = useRef<((mensaje: MensajeSeñalizacionWebRTC) => Promise<void>) | null>(null);
 ...
 if (event.candidate && sendWebRTCMessageRef.current) {
 sendWebRTCMessageRef.current({
 type: 'broadcast',
 event: 'ice-candidate',
 payload: { candidate: event.candidate, to: peerId, from: session?.user?.id }
 }).catch((err: unknown) => {
 const message = err instanceof Error ? err.message : String(err);
 log.error('Failed to send ICE candidate', { error: message, peerId });
 });
 }
 };

 pc.ontrack = (event) => {
 log.debug('Received remote track', { peerId });
 ...
 log.debug('WebRTC connection state changed', { peerId, state: pc.connectionState });
 ...
 if (sendWebRTCMessageRef.current) {
 sendWebRTCMessageRef.current({
 type: 'broadcast',
 event: 'answer',
 ...
 const message = err instanceof Error ? err.message : String(err);
 log.error('Failed to send answer', { error: message, fromId });
 });
 }
 ...
 const message = err instanceof Error ? err.message : String(err);
 log.error('Failed to send offer', { error: message, peerId });
 });
 }
 }, [createPeerConnection, session?.user?.id]);

 // WebRTC Channel - using hook to manage signaling
 const { sendWebRTCMessage } = useVirtualSpaceData(
 activeWorkspace?.id,
 session?.user?.id,
 {
 onOffer: handleOffer,
 ...
 // Store sendWebRTCMessage in ref so it can be used in peer connection callbacks
 useEffect(() => {
 sendWebRTCMessageRef.current = sendWebRTCMessage;
 }, [sendWebRTCMessage]);
 ...
 const message = err instanceof Error ? err.message : String(err);
 log.error('Screen share error', { error: message });
 toggleScreenShare(false);
 if (err instanceof Error && (err.name === 'NotAllowedError' || err.message?.includes('denied'))) {
 ...
 const message = err instanceof Error ? err.message : String(err);
 log.error('Media stream error', { error: message });
 }
 }
 if (activeStreamRef.current) {
 ...
 generatePixelAvatar(key: string, config: AvatarPixelConfig) {
 ...
 const scene = gameRef.current.scene.getScene('CoworkScene') as unknown as CoworkScene;
 ...
 scene.remotePlayers.forEach((container: Phaser.GameObjects.Container, odId: string) => {
 ...
 return (
 <div className={`w-full h-full relative overflow-hidden transition-colors duration-500 ${theme === 'arcade' ? 'bg-black' : (theme === 'space' ? 'bg-[#ECF4FF]' : (theme === 'light' ? 'bg-zinc-100' : 'bg-[#ECF4FF]'))}`}>
 <div ref={containerRef} className="w-full h-full outline-none z-0" />
 <div className={`fixed inset-0 z-10 bg-white/70 backdrop-blur-md transition-opacity duration-700 pointer-events-none ${currentUser.isPrivate ? 'opacity-100' : 'opacity-0'}`} />
 ...
 onTriggerReaction={(emoji: string) => { const scene = gameRef.current?.scene.getScene('CoworkScene') as unknown as CoworkScene; if (scene?.floatEmoji) scene.floatEmoji(emoji); }}
 />
 )}
 </div>
 );
