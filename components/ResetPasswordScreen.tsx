...
export const ResetPasswordScreen: React.FC<ResetPasswordScreenProps> = ({ errorFromHash = false, errorDescription }) => {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (session) {
        // Hay sesión activa; el evento PASSWORD_RECOVERY llegará en breve
        // Si en 3s no llega, asumimos que no es recovery válido
        setTimeout(() => {
        ...
        <div className="relative backdrop-blur-xl bg-white/[0.03] border border-white/[0.08] rounded-[36px] p-6 shadow-2xl">
          ...
          {state === 'error_token' && (
            <div className="space-y-5 text-center animate-in fade-in">
              ...
              <div>
                <h1 className="text-xl font-black text-transparent bg-clip-text bg-gradient-to-r from-white via-red-200 to-white mb-2">
              ...
              <button
                ...
                className="relative w-full group overflow-hidden bg-gradient-to-r from-violet-600 via-fuchsia-600 to-cyan-500 text-white py-3 rounded-xl font-black text-xs uppercase tracking-[0.15em] transition-all shadow-2xl shadow-violet-600/30 active:scale-[0.98]"
              >
              ...
              <form id="reset-password-form" onSubmit={handleSubmit} className="space-y-3">
                ...
                <div className="space-y-1.5">
                  <div className="relative group">
                    ...
                    <input
                      ...
                      className="w-full bg-black/40 border border-white/5 rounded-xl pl-11 pr-11 py-3.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/50 focus:border-violet-500/50 transition-all placeholder:text-zinc-700 text-white"
                    />
                    <button
                      type="button"
                  ...
                  {newPassword && (
                    <div className="space-y-1 animate-in fade-in">
                      ...
                      <p className={`text-[9px] font-bold uppercase tracking-widest ${strength >= 4 ? 'text-emerald-400' : strength >= 3 ? 'text-yellow-400' : 'text-red-400'}`}>
                ...
                <div className="relative group">
                  ...
                  <input
                    ...
                    autoComplete="new-password"
                    className={`w-full bg-black/40 border rounded-xl pl-11 pr-11 py-3.5 text-sm focus:outline-none focus:ring-2 transition-all placeholder:text-zinc-700 text-white ${
                      confirmPassword && confirmPassword !== newPassword
                        ? 'border-red-500/40 focus:ring-red-500/30 focus:border-red-500/40'
                        : 'border-white/5 focus:ring-violet-500/50 focus:border-violet-500/50'
                    }`}
                  />
                  <button
                    type="button"
                ...
                <div className="p-3 bg-white/[0.02] border border-white/5 rounded-xl">
                  <p className="text-[9px] font-bold uppercase tracking-wider text-zinc-500 mb-1.5">Requisitos:</p>
                  <ul className="space-y-1">
                    {[
                      ...
                      <li key={req.label} className={`flex items-center gap-1.5 text-[9px] font-medium transition-colors ${req.ok ? 'text-emerald-400' : 'text-zinc-600'}`}>
                        <span>{req.ok ? '✓' : '○'}</span>
                        {req.label}
                      </li>
                    ))}
          ...
          {state === 'success' && (
            <div className="space-y-5 text-center animate-in fade-in duration-500">
              <div className="relative mx-auto w-16 h-16">
                ...
                <div className="relative w-16 h-16 bg-gradient-to-br from-emerald-500 to-cyan-500 rounded-full flex items-center justify-center">
              ...
              <div>
                <h1 className="text-xl font-black text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-cyan-400 mb-2">
