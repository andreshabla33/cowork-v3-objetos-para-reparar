...
const GLTFAvatarInner: React.FC<GLTFAvatarProps> = ({
  ...
  useEffect(() => {
    const textureUrl = avatarConfig?.textura_url;
    if (!textureUrl) {
  ...
  useFrame(() => {
    ...
    if (isSitting) {
      ...
      if (sitMeasureFrames.current % 120 === 0) {
      ...
      if (sitMeasureFrames.current >= 5) {
        ...
        if (delta > 0.02 || lastReportedSitHipY.current < 0) {
        ...
        // Solo enviar a Player3D cuando es la pose estable 'sit'
        if (animationState === 'sit' && sitMeasureFrames.current >= 10) {
          onMetricasAvatarComputadas?.({
