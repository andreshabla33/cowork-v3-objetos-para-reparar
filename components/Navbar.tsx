...
export const AvatarPreview: React.FC<{
 ...
 <div className="absolute bottom-[2px] w-[70%] h-[10%] bg-white/70 rounded-full blur-[1px]" />
 ...
 <div className="bg-white/60" style={{ width: p * 4.5, height: p * 3, border: `${p/2}px solid #000`, borderTop: 'none' }} />
 <div className="bg-white/60" style={{ width: p * 4.5, height: p * 3, border: `${p/2}px solid #000`, borderTop: 'none' }} />
 </div>
 </div>
 );
...
export const Navbar: React.FC<NavbarProps> = ({ activeTab, setActiveTab, onVibenToggle }) => {
 ...
 { id: 'dark', label: t('theme.dark', currentLang), icon: '🌑', color: 'bg-white/60' },
 ...
 return (
 <nav className={`h-16 border-b transition-all duration-500 flex items-center justify-between px-6 z-50 ${theme === 'dark' ? 'bg-white/60 border-[rgba(46,150,245,0.14)] text-[#0B2240]' : theme === 'light' ? 'bg-white/80 border-zinc-200 text-[#0B2240]' : theme === 'space' ? 'bg-[#ECF4FF]/80 border-indigo-900 text-indigo-100' : 'bg-[#0B2240]/35 backdrop-blur-[10px] border-[#00ff41]/30 text-[#00ff41]'} backdrop-blur-md`}>
 <div className="flex items-center gap-6">
 <div className="flex items-center gap-2 cursor-pointer" onClick={() => setActiveTab('space')}>
 <div className={`w-8 h-8 rounded-lg flex items-center justify-center font-black text-[#0B2240] italic shadow-lg ${theme === 'arcade' ? 'bg-[#00ff41] text-black shadow-[#00ff41]/20' : 'bg-indigo-600 shadow-indigo-500/20'}`}>C</div>
 <span className="font-bold text-lg tracking-tighter uppercase">Cowork</span>
 </div>
 <div className={`flex items-center gap-1 p-1 border ${theme === 'light' ? 'bg-zinc-100 border-zinc-200' : 'bg-white/50 border-[rgba(46,150,245,0.14)]'} rounded-lg`}>
 {(['space', 'tasks', 'builder', 'avatar', 'settings'] as const).map(tab => (
 <button key={tab} onClick={() => setActiveTab(tab)} className={`px-3 py-1 rounded-md text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === tab ? (theme === 'arcade' ? 'bg-[#00ff41] text-black' : 'bg-indigo-600 text-[#0B2240] shadow-md') : 'opacity-60 hover:opacity-100'}`}>{tab}</button>
 ))}
 </div>
 </div>
 <div className="flex items-center gap-4">
 {/* Theme Selector Icon Buttons */}
 <div className={`flex items-center gap-1 p-1 border rounded-full ${theme === 'light' ? 'bg-zinc-100 border-zinc-200' : 'bg-white/50 border-[rgba(46,150,245,0.14)]'}`}>
 {themes.map(t => (
 <button 
 key={t.id} 
 onClick={() => setTheme(t.id)} 
 title={t.label} 
 className={`w-7 h-7 rounded-full flex items-center justify-center transition-all text-xs border-2 ${theme === t.id ? (theme === 'arcade' ? 'border-[#00ff41] bg-[#00ff41]/20 scale-110' : 'border-indigo-500 bg-[rgba(46,150,245,0.08)] scale-110') : 'border-transparent opacity-40 hover:opacity-100'}`}
 >
 {t.icon}
 </button>
 ))}
 ...
 <div className={`flex items-center gap-3 pl-4 border-l ${theme === 'light' ? 'border-zinc-200' : 'border-[rgba(46,150,245,0.14)]'}`}>
 <UserAvatar
