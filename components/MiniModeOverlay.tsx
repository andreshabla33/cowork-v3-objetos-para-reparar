...
const STATUS_OPTIONS = [
 ...
 { value: PresenceStatus.AWAY, label: 'Ausente', color: 'bg-[#2E96F5]' },
 { value: PresenceStatus.DND, label: 'No molestar', color: 'bg-[#2E96F5]' },
];

const statusColorMap: Record<string, string> = {
 [PresenceStatus.AVAILABLE]: 'bg-green-500',
 [PresenceStatus.BUSY]: 'bg-red-500',
 [PresenceStatus.AWAY]: 'bg-[#2E96F5]',
 [PresenceStatus.DND]: 'bg-[#2E96F5]',
};
...
export const MiniModeOverlay: React.FC = () => {
 ...
 className="relative w-11 h-11 rounded-full bg-[#0B2240]/35 backdrop-blur-[10px] backdrop-blur-2xl border border-[rgba(46,150,245,0.14)] shadow-2xl flex items-center justify-center cursor-pointer hover:scale-110 transition-transform duration-200 hover:border-[rgba(46,150,245,0.3)]/50"
 title="Expandir Mini Mode"
 >
 {currentUser.profilePhoto ? (
 <img src={currentUser.profilePhoto} alt="" className="w-7 h-7 rounded-full object-cover" />
 ) : (
 <div className="w-6 h-6 rounded-full bg-gradient-to-br from-[#4FB0FF] to-[#2E96F5] flex items-center justify-center text-[8px] font-black text-white">
 {currentUser.name?.[0]?.toUpperCase() || 'U'}
 </div>
 )}
 <div className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-white ${statusColorMap[currentStatus]}`} />
 {onlineUsers.length > 0 && (
 <div className="absolute -top-1 -left-1 w-4 h-4 rounded-full bg-[#2E96F5] border-2 border-white flex items-center justify-center text-[7px] font-bold text-white">
 {onlineUsers.length}
 </div>
 )}
 </button>
 <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
 <div className="bg-[#0B2240]/35 backdrop-blur-[10px] text-[#0B2240] text-[9px] font-medium px-2 py-1 rounded-lg border border-[rgba(46,150,245,0.14)] whitespace-nowrap">
 ...
 >
 <div className="rounded-2xl border border-[rgba(46,150,245,0.14)] bg-[#0B2240]/35 backdrop-blur-[10px] backdrop-blur-2xl shadow-2xl shadow-[rgba(46,150,245,0.15)]">
 
 {/* Header draggable */}
 <div
 className="flex items-center justify-between px-3 py-2.5 cursor-grab active:cursor-grabbing bg-white/50 rounded-t-2xl"
 onMouseDown={handleMouseDown}
 >
 ...
 <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-[#4FB0FF] to-[#2E96F5] flex items-center justify-center text-[10px] font-black text-white">
 {currentUser.name?.[0]?.toUpperCase() || 'U'}
 </div>
 )}
 <div className={`absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border-2 border-white ${statusColorMap[currentStatus]}`} />
 </div>
 <div>
 <p className="text-[10px] font-bold text-[#1B3A5C] leading-none">{currentUser.name}</p>
 <p className="text-[8px] text-[#9CB0CA] mt-0.5">{onlineUsers.length} online</p>
 </div>
 </div>
 <div className="flex items-center gap-1">
 <button onClick={handleGoToChat} className="p-1.5 rounded-lg bg-white/50 text-[#9CB0CA] hover:bg-blue-600/30 hover:text-blue-400 transition-colors" title="Ir al Chat">
 <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" /></svg>
 </button>
 <button onClick={() => setCollapsed(true)} className="p-1.5 rounded-lg bg-white/50 text-[#9CB0CA] hover:bg-[rgba(46,150,245,0.08)] hover:text-[#4A6485] transition-colors" title="Minimizar">
 ...
 <div className="border-t border-[rgba(46,150,245,0.14)] relative">
 <MiniMapCanvas currentUser={currentUser} onlineUsers={onlineUsers} />
 {/* Badge de usuarios online */}
 <div className="absolute top-1.5 right-1.5 flex items-center gap-1 bg-[#0B2240]/35 backdrop-blur-[10px] backdrop-blur-sm rounded-full px-1.5 py-0.5">
 <div className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse" />
 <span className="text-[7px] text-[#6B83A0] font-medium">{onlineUsers.length} online</span>
 </div>
 </div>

 {/* Controls bar */}
 <div className="flex items-center gap-1.5 px-3 py-2 border-t border-[rgba(46,150,245,0.14)] rounded-b-2xl">
 <button onClick={toggleMic} className={`p-1.5 rounded-lg transition-colors ${currentUser.isMicOn ? 'bg-[rgba(46,150,245,0.08)] text-[#0B2240]' : 'bg-red-500/20 text-red-400'}`} title={currentUser.isMicOn ? 'Silenciar' : 'Activar mic'}>
 ...
 <button onClick={toggleCamera} className={`p-1.5 rounded-lg transition-colors ${currentUser.isCameraOn ? 'bg-[rgba(46,150,245,0.08)] text-[#0B2240]' : 'bg-red-500/20 text-red-400'}`} title={currentUser.isCameraOn ? 'Apagar cámara' : 'Encender cámara'}>
 <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 ...
 className="flex items-center gap-1 px-2 py-1 rounded-lg bg-white/50 hover:bg-[rgba(46,150,245,0.08)] transition-colors"
 title="Cambiar estado"
 >
 <div className={`w-2 h-2 rounded-full ${statusColorMap[currentStatus]}`} />
 <span className="text-[8px] font-bold text-[#9CB0CA] uppercase tracking-wider">
 {STATUS_OPTIONS.find(s => s.value === currentStatus)?.label || 'Disponible'}
 </span>
 <svg className="w-2 h-2 text-[#9CB0CA]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M19 9l-7 7-7-7" /></svg>
 </button>

 <div className="flex-1" />

 <button onClick={handleGoToSpace} className="flex items-center gap-1 px-2 py-1.5 rounded-lg bg-[#2E96F5]/20 hover:bg-[#2E96F5]/40 text-[#1E86E5] transition-colors" title="Volver al espacio">
 ...
 <div
 className="fixed z-[10001] bg-[#0B2240]/35 backdrop-blur-[10px] backdrop-blur-xl border border-[rgba(46,150,245,0.14)] rounded-xl shadow-2xl"
 ...
 className={`w-full flex items-center gap-2.5 px-3 py-2.5 text-left hover:bg-[rgba(46,150,245,0.08)] transition-colors first:rounded-t-xl last:rounded-b-xl ${currentStatus === opt.value ? 'bg-[#2E96F5]/20' : ''}`}
 >
 <div className={`w-2.5 h-2.5 rounded-full ${opt.color}`} />
 <span className="text-[10px] font-medium text-[#1B3A5C]">{opt.label}</span>
 </button>
 ))}
 </div>
 </>
