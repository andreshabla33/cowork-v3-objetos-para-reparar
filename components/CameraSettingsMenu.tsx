...
import { loadCameraSettings, saveCameraSettings, type CameraSettings } from '@/modules/realtime-room';
...
export const CameraSettingsMenu: React.FC<CameraSettingsMenuProps> = ({
 ...
 setSettings(prev => ({
 ...prev,
 selectedCameraId: currentDeviceId || videoDevices[0].id
 }));
 }
 } catch (err) {
 ...
 <div
 ref={menuRef}
 className="absolute z-[100] w-72 bg-white/60/95 backdrop-blur-xl rounded-xl border border-[rgba(46,150,245,0.14)] shadow-2xl overflow-hidden animate-in fade-in slide-in-from-bottom-2 duration-200"
 style={{
 ...
 >
 {/* Header */}
 <div className="px-4 py-3 border-b border-[rgba(46,150,245,0.14)] flex items-center justify-between">
 <span className="text-sm font-semibold text-[#0B2240]">Configuración de cámara</span>
 <button
 onClick={onClose}
 className="w-6 h-6 rounded-full flex items-center justify-center hover:bg-[rgba(46,150,245,0.08)] transition-colors"
 >
 ...
 <label className="text-xs font-medium text-[#4A6485] uppercase tracking-wide">
 Seleccionar cámara
 </label>
 ...
 className="w-full bg-white/50 border border-[rgba(46,150,245,0.14)] rounded-lg px-3 py-2.5 text-sm text-[#0B2240] appearance-none cursor-pointer hover:bg-[rgba(46,150,245,0.08)]0 transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-500/50"
 >
 {cameras.length === 0 ? (
 ...
 <span className="text-sm text-[#0B2240]">Ocultar mi vista</span>
 </div>
 <button
 onClick={() => updateSettings({ hideSelfView: !settings.hideSelfView })}
 className={`w-11 h-6 rounded-full transition-colors relative ${
 settings.hideSelfView ? 'bg-indigo-600' : 'bg-white/50'
 }`}
 >
 ...
 <span className="text-sm text-[#0B2240]">Efectos de fondo</span>
 </div>
 ...
 <span className="text-[10px] text-[#6B83A0]">Ninguno</span>
 <span className="text-[10px] text-[#6B83A0]">Desenfoque</span>
 <span className="text-[10px] text-[#6B83A0]">Imagen</span>
 </div>
 ...
 <span className="text-sm text-[#0B2240]">Espejo de video</span>
 </div>
 <button
 onClick={() => updateSettings({ mirrorVideo: !settings.mirrorVideo })}
 className={`w-11 h-6 rounded-full transition-colors relative ${
 settings.mirrorVideo ? 'bg-indigo-600' : 'bg-white/50'
 }`}
 >
 <div className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-transform ${
 ...
 {/* Info */}
 <div className="text-[10px] text-[#9CB0CA] text-center pt-2">
