...
export const MiembrosView: React.FC = () => {
 ...
 const map: any = { 
 super_admin: isArcade ? 'bg-[#00ff41]/20 text-[#00ff41] border border-[#00ff41]' : 'bg-[#2E96F5]', 
 ...
 <p className={`text-[10px] font-black uppercase tracking-[0.3em] mt-2 ${isArcade ? 'text-[#00ff41]/60' : 'text-[#4A6485]'}`}>Gestiona el acceso de tu organización</p>
 </div>
 <div className="flex items-center gap-3">
 {esCeoCoo && (
 <div className={`flex rounded-xl overflow-hidden border ${isArcade ? 'border-[#00ff41]/30' : 'border-[rgba(46,150,245,0.14)]'}`}>
 <button onClick={() => setFiltroTiempo('hoy')} className={`px-4 py-2 text-[9px] font-black uppercase tracking-widest transition-all ${filtroTiempo === 'hoy' ? (isArcade ? 'bg-[#00ff41] text-black' : 'bg-indigo-600 text-[#0B2240]') : 'bg-transparent opacity-50 hover:opacity-100'}`}>Hoy</button>
 <button onClick={() => setFiltroTiempo('semana')} className={`px-4 py-2 text-[9px] font-black uppercase tracking-widest transition-all ${filtroTiempo === 'semana' ? (isArcade ? 'bg-[#00ff41] text-black' : 'bg-indigo-600 text-[#0B2240]') : 'bg-transparent opacity-50 hover:opacity-100'}`}>7 días</button>
 </div>
 )}
 {isAdmin && (
 <button 
 ...
 isArcade ? 'bg-[#00ff41] text-black shadow-[#00ff41]/30' : 'bg-indigo-600 text-[#0B2240] shadow-indigo-600/20'
 }`}
 >
 Invitar Usuario
 </button>
 ...
 <section>
 <div className={`overflow-hidden rounded-[32px] border ${isArcade ? 'border-[#00ff41]/30 bg-black' : 'border-[rgba(46,150,245,0.14)] bg-white/60'} shadow-2xl`}>
 <table className="w-full text-left border-collapse">
 <thead>
 <tr className={`${isArcade ? 'bg-[#00ff41]/5' : 'bg-white/70'}`}>
 <th className={`p-6 text-[10px] font-black uppercase tracking-[0.2em] ${isArcade ? 'text-[#00ff41]/40' : 'text-[#4A6485]'}`}>Usuario</th>
 <th className={`p-6 text-[10px] font-black uppercase tracking-[0.2em] ${isArcade ? 'text-[#00ff41]/40' : 'text-[#4A6485]'}`}>Rol</th>
 <th className={`p-6 text-[10px] font-black uppercase tracking-[0.2em] ${isArcade ? 'text-[#00ff41]/40' : 'text-[#4A6485]'}`}>Departamento</th>
 {esCeoCoo && (
 <th className={`p-6 text-[10px] font-black uppercase tracking-[0.2em] ${isArcade ? 'text-[#00ff41]/40' : 'text-[#4A6485]'}`}>
 Tiempo Conectado
 </th>
 )}
 <th className={`p-6 text-[10px] font-black uppercase tracking-[0.2em] ${isArcade ? 'text-[#00ff41]/40' : 'text-[#4A6485]'}`}>Acciones</th>
 </tr>
 </thead>
 ...
 return (
 <tr key={m.id} className={`${isArcade ? 'hover:bg-[#00ff41]/5' : 'hover:bg-white/50'} transition-colors`}>
 <td className="p-6">
 ...
 <div className="absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 bg-green-500 rounded-full border-2 border-white animate-pulse" title="Conectado ahora" />
 )}
 </div>
 <div>
 <p className={`text-sm font-black uppercase tracking-widest ${isArcade ? 'text-[#00ff41]' : 'text-[#0B2240]'}`}>{m.usuario?.nombre}</p>
 <p className={`text-[10px] font-bold ${isArcade ? 'text-[#00ff41]/40' : 'text-[#4A6485]'}`}>{m.usuario?.email}</p>
 </div>
 </div>
 </td>
 <td className="p-6">
 ...
 <td className="p-6">
 <p className={`text-[10px] font-black uppercase tracking-widest ${isArcade ? 'text-[#00ff41]/60' : 'text-[#4A6485]'}`}>
 ...
 <div className={`w-20 h-2 rounded-full overflow-hidden ${isArcade ? 'bg-[#00ff41]/10' : 'bg-white/50'}`}>
 <div 
 className={`h-full rounded-full transition-all ${
 tiempoMins > 360 ? (isArcade ? 'bg-[#00ff41]' : 'bg-green-500') :
 tiempoMins > 120 ? (isArcade ? 'bg-[#00ff41]/70' : 'bg-indigo-500') :
 tiempoMins > 0 ? (isArcade ? 'bg-[#00ff41]/40' : 'bg-[#2E96F5]') :
 'bg-transparent'
 }`}
 ...
 tiempoMins > 0 ? (isArcade ? 'text-[#00ff41]' : 'text-[#0B2240]') : 'text-[#6B83A0]'
 }`}>
 ...
 <section>
 <h3 className={`text-[10px] font-black uppercase tracking-[0.3em] mb-6 ${isArcade ? 'text-[#00ff41]/60' : 'text-[#6B83A0]'}`}>Invitaciones Pendientes</h3>
 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
 {invitaciones.map(inv => (
 <div key={inv.id} className={`p-8 rounded-[40px] border shadow-2xl transition-all hover:scale-[1.02] ${isArcade ? 'bg-black border-[#00ff41]/20' : 'bg-white/60 border-[rgba(46,150,245,0.14)]'}`}>
 ...
 <span className={`text-[8px] font-black uppercase tracking-widest px-2 py-1 rounded-md ${isArcade ? 'bg-[#00ff41]/5 text-[#00ff41]/40' : 'bg-white/70 text-[#6B83A0]'}`}>
 Pendiente
 </span>
 </div>
 <p className={`text-xs font-black uppercase tracking-widest mb-1 truncate ${isArcade ? 'text-[#00ff41]' : 'text-[#0B2240]'}`}>{inv.email}</p>
 <p className={`text-[9px] font-bold uppercase tracking-widest mb-6 ${isArcade ? 'text-[#00ff41]/40' : 'text-[#4A6485]'}`}>{inv.rol}</p>
