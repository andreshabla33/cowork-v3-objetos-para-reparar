...
export const PanelDetalleTerreno: React.FC<PanelDetalleTerrenoProps> = ({
 ...
 {/* Header */}
 <div className={`p-6 bg-gradient-to-br ${tier.bgGradient} border-b border-[rgba(46,150,245,0.14)]`}>
 <div className="flex items-start justify-between mb-4">
 <div className="flex-1">
 {terreno.destacado && (
 <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-[rgba(46,150,245,0.12)] border border-[rgba(46,150,245,0.3)]/30 rounded-full text-[10px] font-bold text-[#1E86E5] uppercase tracking-wider mb-2">
 <Star className="w-3 h-3" /> Destacado
 </span>
 )}
 <h2 className="text-xl font-bold text-[#0B2240]">{terreno.nombre}</h2>
 <div className="flex items-center gap-2 mt-1">
 <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-bold ${tier.textColor} bg-white/50 border ${tier.borderColor}`}>
 {tier.label}
 </span>
 <span className="text-xs text-[#4A6485]">{tier.subtitulo}</span>
 </div>
 </div>
 <button
 onClick={onCerrar}
 ...
 <div>
 <p className="text-3xl font-black text-[#0B2240]">
 {formatPrecio(terreno.precio_mensual)}
 <span className="text-sm font-normal text-[#4A6485]">/mes</span>
 </p>
 </div>
 ...
 {terreno.descripcion && (
 <div>
 <p className="text-sm text-[#1B3A5C] leading-relaxed">{terreno.descripcion}</p>
 </div>
 )}

 {/* Dimensiones */}
 <div className="bg-white/50 rounded-xl p-4 border border-[rgba(46,150,245,0.14)]">
 <h4 className="text-xs font-bold text-[#4A6485] uppercase tracking-wider mb-3">Dimensiones</h4>
 <div className="grid grid-cols-2 gap-3">
 <div className="flex items-center gap-2">
 <MapPin className="w-4 h-4 text-[#4A6485]" />
 <div>
 <p className="text-xs text-[#4A6485]">Ubicación</p>
 <p className="text-sm font-medium text-[#0B2240]">({terreno.posicion_x}, {terreno.posicion_y})</p>
 </div>
 </div>
 <div className="flex items-center gap-2">
 <div className="w-4 h-4 rounded border border-[rgba(46,150,245,0.14)] flex items-center justify-center">
 <div className="w-2 h-2 bg-[#9CB0CA] rounded-sm" />
 </div>
 <div>
 <p className="text-xs text-[#4A6485]">Tamaño</p>
 <p className="text-sm font-medium text-[#0B2240]">{terreno.ancho} × {terreno.alto} u²</p>
 </div>
 </div>
 </div>
 </div>
 ...
 {/* Features incluidos */}
 <div className="bg-white/50 rounded-xl p-4 border border-[rgba(46,150,245,0.14)]">
 <h4 className="text-xs font-bold text-[#4A6485] uppercase tracking-wider mb-3">Incluido en el terreno</h4>
 ...
 <button
 disabled
 className="w-full py-3.5 px-6 bg-white/50 text-[#4A6485] font-bold rounded-xl text-sm cursor-not-allowed"
 >
...
const FeatureRow: React.FC<{
 ...
 <div className="flex items-center gap-2 text-[#4A6485]">
 {icon}
 <span className="text-xs">{label}</span>
 </div>
 <span className={`text-xs font-medium ${activo === false ? 'text-[#6B83A0]' : 'text-[#0B2240]'}`}>
 {valor}
 </span>
 </div>
);
