...
export const VibenAssistant: React.FC<VibenAssistantProps> = ({ onClose }) => {
 ...
 return (
 <div ref={containerRef} className={`flex flex-col rounded-2xl overflow-hidden transition-all duration-300 ease-in-out w-full border border-[var(--cw-glass-border)] shadow-[var(--cw-shadow-floating)] bg-[var(--cw-glass-bg-strong)] backdrop-blur-[28px] ${isMinimized ? 'h-[52px]' : 'h-[520px] max-h-[80vh]'}`}>
 {/* Header */}
 <div 
 onClick={() => setIsMinimized(!isMinimized)}
 className="relative overflow-hidden bg-white/80 backdrop-blur-xl p-3.5 flex items-center justify-between z-10 cursor-pointer group shrink-0 border-b border-[var(--cw-glass-border)]"
 >
 <div className="flex items-center gap-3">
 <div className="w-8 h-8 rounded-xl bg-[#ECF4FF] flex items-center justify-center text-base border border-[rgba(46,150,245,0.16)]">
 <svg className="w-4 h-4 text-[#1E86E5]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9.75 3.104v5.714a2.25 2.25 0 01-.659 1.591L5 14.5M9.75 3.104c-.251.023-.501.05-.75.082m.75-.082a24.301 24.301 0 014.5 0m0 0v5.714c0 .597.237 1.17.659 1.591L19.8 15.3M14.25 3.104c.251.023.501.05.75.082M19.8 15.3l-1.57.393A9.065 9.065 0 0112 15a9.065 9.065 0 00-6.23.693L5 14.5m14.8.8l1.402 1.402c1.232 1.232.65 3.318-1.067 3.611A48.309 48.309 0 0112 21c-2.773 0-5.491-.235-8.135-.687-1.718-.293-2.3-2.379-1.067-3.61L5 14.5" /></svg>
 </div>
 <div>
 <h3 className="text-[12px] font-bold text-[var(--cw-ink-900)]">Mónica AI</h3>
 ...
 className="w-7 h-7 rounded-lg flex items-center justify-center text-[var(--cw-ink-400)] hover:text-[var(--cw-ink-700)] hover:bg-[var(--cw-blue-50)] transition-all"
 title="Limpiar historial"
 >
 <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
 </button>
 <div className={`w-7 h-7 rounded-lg flex items-center justify-center text-[var(--cw-ink-400)] transition-transform duration-300 ${isMinimized ? 'rotate-180' : ''}`}>
 <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7" /></svg>
 </div>
 <button 
 onClick={(e) => { e.stopPropagation(); onClose(); }} 
 className="w-7 h-7 rounded-lg flex items-center justify-center text-[var(--cw-ink-400)] hover:text-red-500 hover:bg-red-50 transition-all"
 >
 ...
 <>
 {/* Mensajes */}
 <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3 bg-[var(--cw-page-bg)] scroll-smooth custom-scrollbar">
 {messages.map((m, i) => (
 <div key={i} className={`flex gap-2 ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
 {m.role === 'monica' && (
 <div className="w-6 h-6 rounded-lg bg-[#ECF4FF] flex items-center justify-center shrink-0 mt-1 border border-[rgba(46,150,245,0.16)]">
 <svg className="w-3 h-3 text-[#1E86E5]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9.75 3.104v5.714a2.25 2.25 0 01-.659 1.591L5 14.5M9.75 3.104c-.251.023-.501.05-.75.082m.75-.082a24.301 24.301 0 014.5 0m0 0v5.714c0 .597.237 1.17.659 1.591L19.8 15.3M14.25 3.104c.251.023.501.05.75.082M19.8 15.3l-1.57.393A9.065 9.065 0 0112 15a9.065 9.065 0 00-6.23.693L5 14.5m14.8.8l1.402 1.402c1.232 1.232.65 3.318-1.067 3.611A48.309 48.309 0 0112 21c-2.773 0-5.491-.235-8.135-.687-1.718-.293-2.3-2.379-1.067-3.61L5 14.5" /></svg>
 </div>
 )}
 <div className={`max-w-[80%] rounded-2xl px-3.5 py-2.5 text-[12px] leading-relaxed ${
 m.role === 'user' 
 ? 'bg-[var(--cw-blue-500)] text-[#0B2240] rounded-br-md shadow-md shadow-blue-200/40' 
 : 'bg-white/80 text-[var(--cw-ink-700)] rounded-bl-md border border-[var(--cw-glass-border)]'
 }`}>
 <p className="whitespace-pre-wrap">{m.text}</p>
 <div className={`text-[8px] mt-1.5 opacity-50 font-medium ${m.role === 'user' ? 'text-right' : 'text-left'}`}>
 ...
 {streamingText ? (
 <div className="max-w-[80%] rounded-2xl px-3.5 py-2.5 text-[12px] leading-relaxed bg-white/80 text-[var(--cw-ink-700)] rounded-bl-md border border-[var(--cw-glass-border)]">
 <p className="whitespace-pre-wrap">{streamingText}</p>
 <span className="inline-block w-1.5 h-3 bg-[#2E96F5] animate-pulse ml-0.5" />
 </div>
 ) : (
 <div className="bg-white/80 border border-[var(--cw-glass-border)] rounded-2xl rounded-bl-md px-4 py-3 flex items-center gap-1.5">
 <span className="w-1.5 h-1.5 bg-[#2E96F5] rounded-full animate-bounce"></span>
 <span className="w-1.5 h-1.5 bg-[#2E96F5] rounded-full animate-bounce [animation-delay:0.15s]"></span>
 <span className="w-1.5 h-1.5 bg-[#4FB0FF] rounded-full animate-bounce [animation-delay:0.3s]"></span>
 </div>
 )}
 <button 
 ...
 {/* Input */}
 <div className="p-3 bg-white/70 backdrop-blur-xl border-t border-[var(--cw-glass-border)] shrink-0">
 ...
 className="flex-1 bg-white/80 border border-[var(--cw-glass-border)] rounded-xl px-4 py-2.5 text-[12px] text-[var(--cw-ink-700)] focus:outline-none focus:border-[var(--cw-blue-400)] focus:ring-2 focus:ring-blue-100 transition-all placeholder:text-[var(--cw-ink-400)]"
 />
 <button 
 onClick={handleSend} 
 disabled={isTyping || !input.trim()} 
 className="w-10 h-10 rounded-xl bg-[#2E96F5] hover:bg-[#2E96F5] disabled:opacity-30 disabled:cursor-not-allowed flex items-center justify-center transition-all shadow-md shadow-[rgba(46,150,245,0.2)]"
 >
