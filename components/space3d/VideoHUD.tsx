...
import type { SpaceVideoHudLayoutModel } from '@/modules/realtime-room';
...
export const VideoHUD: React.FC<VideoHUDProps> = ({
  ...
  const expandedRemoteParticipant = expandedRemoteParticipantId ? layoutSnapshot.usersById.get(expandedRemoteParticipantId) : null;
  ...
  return (
    <>
              ...
              {(expandedId === 'local' && (localVideoTrack || localExpandedStream)) || (expandedId === 'screen' && screenStream) || (expandedId?.startsWith('screen-') && layoutSnapshot.remoteScreenStreamsById.get(expandedId.replace('screen-', ''))) || (expandedId && layoutSnapshot.remoteVideoStreamsById.get(expandedId)) ? (
                ...
                <div className="w-full h-full flex items-center justify-center">
                  <div className="w-32 h-32 rounded-full bg-zinc-800 flex items-center justify-center text-6xl font-black text-white">
                    {expandedId === 'local' ? userName.charAt(0) : expandedRemoteParticipant?.name.charAt(0) || '?'}
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="absolute top-4 left-4 right-4 flex items-center justify-between">
              <div className="bg-white/5 backdrop-blur-2xl px-4 py-2 rounded-2xl border border-white/10">
                <span className="text-sm font-medium text-white/90">
                  {expandedId === 'local' ? 'Tu cámara' : expandedId === 'screen' ? 'Tu pantalla' : expandedId?.startsWith('screen-') ? `${expandedRemoteParticipant?.name || 'Usuario'} - Pantalla` : expandedRemoteParticipant?.name || 'Usuario'}
                </span>
              </div>
              <button 
      ...
      <div className={`absolute left-1/2 top-24 -translate-x-1/2 pointer-events-auto z-50 transition-all duration-500 ${
        ...
        {layoutSnapshot.remoteCameraTiles.map((tile) => {
          const { participantId, user: u, videoStream: remoteStream, isSpeaking, distance, hasVideoStream: hasRemoteStream, shouldShowCamera: shouldShowRemoteCam, isHandRaised } = tile;
          const isWaving = waveAnimation === participantId;

          return (
            <div key={participantId} className={`relative bg-zinc-900 rounded-[28px] overflow-hidden shadow-2xl group transition-all duration-300 ${
              useGridLayout ? 'w-[200px] h-[130px]' : 'w-52 h-36'
              ...
              <div className="absolute top-3 left-3 right-3 flex items-center justify-between">
                ...
                <div className="flex items-center gap-1.5">
                  {isHandRaised && (
                    <div className="bg-sky-500/80 backdrop-blur-md px-1.5 py-0.5 rounded text-[9px] text-white">
                      ✋
                    </div>
                  )}
              ...
              <div className="absolute bottom-3 left-3 right-3 flex justify-between items-center opacity-0 group-hover:opacity-100 transition-all">
                ...
                <button 
                  onClick={() => {
                    onWaveUser(participantId);
                    setWaveAnimation(participantId);
        ...
        {layoutSnapshot.remoteScreenShareTiles.map((tile) => {
          const { participantId, user: u, screenStream: remoteScreen, cameraStream: remoteStream, hasCameraPreview: hasActiveCamera, isHandRaised } = tile;

          return (
            <React.Fragment key={`screen-group-${participantId}`}>
              ...
              <div className="relative bg-black rounded-[28px] overflow-hidden border border-green-500/30 shadow-2xl group w-80 h-48">
                <StableVideo stream={remoteScreen} className="w-full h-full object-contain" speakerDeviceId={speakerDeviceId} />
                ...
                {isHandRaised && (
                  <div className="absolute top-2 left-[88px] bg-sky-500/80 backdrop-blur-sm px-1.5 py-0.5 rounded-md opacity-80 group-hover:opacity-100 transition-opacity">
                ...
                <button onClick={() => setExpandedId(`screen-${participantId}`)} className="absolute bottom-2 right-2 w-7 h-7 rounded-lg flex items-center justify-center bg-white/10 backdrop-blur-sm text-white/70 opacity-0 group-hover:opacity-100 hover:bg-white/20 transition-all">
                  <IconExpand on={false}/>
                </button>
              </div>
