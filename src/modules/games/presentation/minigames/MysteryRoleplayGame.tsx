...
export const MysteryRoleplayGame: React.FC<MysteryRoleplayGameProps> = ({ onClose }) => {
 ...
 <div><h2 className="text-2xl lg:text-xl font-bold text-[#0B2240]">Mystery Role Play</h2><p className="text-sm lg:text-xs text-[#4A6485]">Resuelve el misterio en equipo</p></div>
 </div>
 ...
 <button onClick={handleStartGame} className="w-full py-4 bg-indigo-600 hover:bg-indigo-500 text-[#0B2240] rounded-xl font-semibold flex items-center justify-center gap-2 transition-colors">
 ...
 <h3 className="text-lg lg:text-base font-semibold text-[#0B2240] mb-4 lg:mb-3">Roles Disponibles</h3>
 <div className="space-y-3">
 {ROLES.map((role) => (
 <div key={role.id} className="p-3 bg-white/60 rounded-xl">
 <div className="flex items-center gap-3">
 <span className="text-2xl lg:text-xl">{role.avatar}</span>
 <div><p className="font-medium text-[#0B2240]">{role.name}</p><p className="text-xs text-[#4A6485]">{role.isSuspect ? 'Sospechoso' : 'Investigador'}</p></div>
 </div>
 </div>
 ))}
 </div>
 ...
 <p className="text-[#4A6485] mb-4">Tu rol secreto es...</p>
 ...
 <h2 className="text-3xl font-bold text-[#0B2240] mb-2">{assignedRole.name}</h2>
 <p className="text-indigo-200">{assignedRole.description}</p>
 </div>
 <div className="p-4 bg-white/60 rounded-xl border border-[rgba(46,150,245,0.14)] mb-6">
 <p className="text-sm text-[#4A6485] mb-2">Tu información secreta:</p>
 <p className="text-[#1B3A5C]">{assignedRole.secretInfo}</p>
 </div>
 <button onClick={handleBeginInvestigation} className="w-full py-4 bg-indigo-600 hover:bg-indigo-500 text-[#0B2240] rounded-xl font-semibold transition-colors">Comenzar a Investigar</button>
 </motion.div>
 </div>
 );
 }
 ...
 <h2 className="text-3xl font-bold text-[#0B2240] mb-2">¡Tiempo de Votar!</h2>
 <p className="text-[#4A6485] mb-8">Basándote en la evidencia y las discusiones, ¿quién crees que es el culpable?</p>

 {!hasVoted ? (
 <div className="space-y-3">
 {ROLES.filter(r => r.isSuspect).map((role) => (
 <motion.button key={role.id} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => setSelectedSuspect(role.id)} className={`w-full p-4 rounded-xl border-2 text-left transition-all ${selectedSuspect === role.id ? 'bg-indigo-500/20 border-indigo-500' : 'bg-white/60 border-[rgba(46,150,245,0.14)] hover:border-[rgba(46,150,245,0.16)]'}`}>
 <div className="flex items-center gap-4">
 <span className="text-3xl">{role.avatar}</span>
 <div className="flex-1"><p className="font-semibold text-[#0B2240]">{role.name}</p><p className="text-sm text-[#4A6485]">{role.alibi}</p></div>
 {selectedSuspect === role.id && <CheckCircle2 className="w-6 h-6 text-indigo-400" />}
 </div>
 </motion.button>
 ))}
 <button onClick={handleVote} disabled={!selectedSuspect} className="w-full py-4 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed text-[#0B2240] rounded-xl font-semibold transition-colors mt-4">Confirmar Voto</button>
 </div>
 ) : (
 <div className="p-6 bg-green-500/10 border border-green-500/30 rounded-xl">
 <CheckCircle2 className="w-8 h-8 text-green-400 mx-auto mb-2" />
 <p className="text-green-400 font-semibold">Voto registrado</p>
 <p className="text-[#4A6485] text-sm">Esperando resultados...</p>
 </div>
 )}
 </motion.div>
 </div>
 ...
 <p className="text-[#4A6485] mt-2">{isCorrect ? 'Excelente trabajo de investigación' : 'El culpable escapó esta vez'}</p>
 </div>

 <div className="p-6 bg-white/60 rounded-xl border border-[rgba(46,150,245,0.14)]">
 <p className="text-sm text-[#4A6485] mb-2">El culpable era:</p>
 <div className="flex items-center justify-center gap-3">
 <span className="text-4xl">{ROLES.find(r => r.id === correctAnswer)?.avatar}</span>
 <span className="text-xl font-bold text-[#0B2240]">{ROLES.find(r => r.id === correctAnswer)?.name}</span>
 </div>
 </div>

 <div className="grid grid-cols-3 gap-4">
 <div className="p-4 bg-white/60 rounded-xl"><p className="text-2xl font-bold text-[#0B2240]">{clues.filter(c => c.revealed).length}</p><p className="text-xs text-[#4A6485]">Pistas reveladas</p></div>
 <div className="p-4 bg-white/60 rounded-xl"><p className="text-2xl font-bold text-[#0B2240]">{chatMessages.length}</p><p className="text-xs text-[#4A6485]">Mensajes</p></div>
 <div className="p-4 bg-white/60 rounded-xl"><p className="text-2xl font-bold text-indigo-400">{isCorrect ? '500' : '100'}</p><p className="text-xs text-[#4A6485]">Puntos</p></div>
 </div>

 <div className="flex gap-4 justify-center">
 <button onClick={() => window.location.reload()} className="px-6 py-3 bg-indigo-600 hover:bg-indigo-500 text-[#0B2240] rounded-xl font-semibold transition-colors">Nuevo Caso</button>
 ...
 <div>
 <p className="text-sm text-[#4A6485]">Investigación en curso</p>
 <div className="flex items-center gap-2"><span className="text-xs text-[#4A6485]">Pistas:</span><span className="text-sm font-bold text-[#0B2240]">{clues.filter(c => c.revealed).length}/{clues.length}</span></div>
 </div>
 </div>
 <div className="flex items-center gap-4">
 ...
 <div className="w-80 border-r border-[rgba(46,150,245,0.14)] bg-white/60 p-4 overflow-auto">
 <h3 className="font-semibold text-[#0B2240] mb-4 flex items-center gap-2"><Fingerprint className="w-4 h-4" />Pistas</h3>
 <div className="space-y-2">
 {clues.map((clue) => (
 <div key={clue.id} onClick={() => !clue.revealed && handleRevealClue(clue.id)} className={`p-3 rounded-xl border cursor-pointer transition-all ${clue.revealed ? 'bg-indigo-500/10 border-indigo-500/30' : 'bg-white/60 border-[rgba(46,150,245,0.14)] hover:border-[rgba(46,150,245,0.16)]'}`}>
 <div className="flex items-center gap-3">
 <span className="text-2xl">{clue.icon}</span>
 <div className="flex-1">
 <p className={`font-medium text-sm ${clue.revealed ? 'text-[#0B2240]' : 'text-[#4A6485]'}`}>{clue.revealed ? clue.title : '???'}</p>
 {clue.revealed && <p className="text-xs text-[#4A6485] mt-1">{clue.description}</p>}
 </div>
 {!clue.revealed && <Lightbulb className="w-4 h-4 text-[#6B83A0]" />}
 </div>
 </div>
 ))}
 </div>
 ...
 <h4 className="font-medium text-[#0B2240] mb-2 flex items-center gap-2"><Star className="w-4 h-4" />Tu Rol</h4>
 <div className="flex items-center gap-2">
 <span className="text-2xl">{assignedRole.avatar}</span>
 <div><p className="text-sm text-[#0B2240]">{assignedRole.name}</p><button onClick={() => setRevealedInfo(!revealedInfo)} className="text-xs text-indigo-400 hover:text-indigo-300">{revealedInfo ? 'Ocultar info' : 'Ver info secreta'}</button></div>
 </div>
 {revealedInfo && <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-2 text-xs text-[#4A6485] p-2 bg-white/50 rounded-lg">{assignedRole.secretInfo}</motion.p>}
 </div>
 </div>
 ...
 {msg.isSystem ? (
 <span className="inline-block px-4 py-2 bg-white/50 rounded-full text-sm text-[#4A6485]">{msg.message}</span>
 ) : (
 <div className={`flex ${msg.sender.startsWith('Tú') ? 'justify-end' : 'justify-start'}`}>
 <div className={`max-w-[70%] px-4 py-3 rounded-2xl ${msg.sender.startsWith('Tú') ? 'bg-indigo-600 text-[#0B2240] rounded-br-md' : 'bg-white/50 text-[#1B3A5C] rounded-bl-md'}`}>
 <p className="text-xs text-[#4A6485] mb-1">{msg.sender}</p>
 <p>{msg.message}</p>
 </div>
 </div>
 )}
 ...
 <div className="p-4 border-t border-[rgba(46,150,245,0.14)]">
 <div className="flex gap-2">
 <input type="text" value={chatInput} onChange={(e) => setChatInput(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && handleSendMessage()} placeholder="Haz una pregunta o comparte tu teoría..." className="flex-1 px-4 py-3 bg-white/50 border border-[rgba(46,150,245,0.16)] rounded-xl text-[#0B2240] placeholder-[#9CB0CA] focus:outline-none focus:border-indigo-500" />
 <button onClick={handleSendMessage} className="px-4 py-3 bg-indigo-600 hover:bg-indigo-500 text-[#0B2240] rounded-xl transition-colors"><Send className="w-5 h-5" /></button>
 </div>
 </div>
 </div>

 <div className="w-72 border-l border-[rgba(46,150,245,0.14)] bg-white/60 p-4">
 <h3 className="font-semibold text-[#0B2240] mb-4">Sospechosos</h3>
 <div className="space-y-2">
 {ROLES.filter(r => r.isSuspect).map((role) => (
 <div key={role.id} className="p-3 bg-white/60 rounded-xl">
 <div className="flex items-center gap-3">
 <span className="text-2xl">{role.avatar}</span>
 <div><p className="font-medium text-[#0B2240] text-sm">{role.name}</p><p className="text-xs text-[#4A6485]">{role.alibi}</p></div>
 </div>
 </div>
 ))}
 </div>
