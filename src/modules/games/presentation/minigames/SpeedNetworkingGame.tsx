...
export const SpeedNetworkingGame: React.FC<SpeedNetworkingGameProps> = ({ onClose }) => {
 ...
 <div><h2 className="text-2xl lg:text-xl font-bold text-[#0B2240]">Speed Networking</h2><p className="text-sm lg:text-xs text-[#4A6485]">Conecta con tus compañeros rápidamente</p></div>
 </div>
 ...
 <h3 className="text-lg lg:text-base font-semibold text-[#0B2240]">Cómo funciona</h3>
 ...
 <button onClick={handleStartGame} className="w-full py-4 bg-pink-600 hover:bg-pink-500 text-[#0B2240] rounded-xl font-semibold flex items-center justify-center gap-2 transition-colors">
 <Shuffle className="w-5 h-5" />Comenzar Networking
 </button>
 </div>

 <div className="w-80 lg:w-64 p-6 lg:p-4 border-l border-[rgba(46,150,245,0.14)]">
 <h3 className="text-lg lg:text-base font-semibold text-[#0B2240] mb-4 lg:mb-3">Participantes</h3>
 <div className="space-y-3">
 <div className="flex items-center gap-3 p-3 bg-pink-500/10 rounded-xl border border-pink-500/30">
 <span className="text-2xl lg:text-xl">👤</span><div><p className="font-medium text-[#0B2240]">Tú</p><p className="text-xs text-pink-400">Jugador</p></div>
 </div>
 {MOCK_PARTICIPANTS.map((p) => (
 <div key={p.id} className="flex items-center gap-3 p-3 bg-white/60 rounded-xl">
 <span className="text-2xl lg:text-xl">{p.avatar}</span><div><p className="font-medium text-[#0B2240]">{p.name}</p><p className="text-xs text-[#4A6485]">{p.role}</p></div>
 </div>
 ))}
 </div>
 </div>
 ...
 <p className="text-[#4A6485] mb-4">Preparando conexiones...</p>
 <motion.div animate={{ scale: [1, 1.2, 1] }} transition={{ repeat: Infinity, duration: 1 }} className="text-6xl">🔗</motion.div>
 <p className="text-[#4A6485] mt-4">Emparejando participantes</p>
 </motion.div>
 </div>
 );
 }
 ...
 <div><h2 className="text-4xl font-bold text-[#0B2240]">¡Networking Completado!</h2><p className="text-[#4A6485] mt-2">Has conectado con {connections.length} compañeros</p></div>

 <div className="grid grid-cols-3 gap-4">
 <div className="p-4 bg-white/60 rounded-xl"><p className="text-3xl font-bold text-[#0B2240]">{conversations.length}</p><p className="text-xs text-[#4A6485]">Conversaciones</p></div>
 <div className="p-4 bg-white/60 rounded-xl"><p className="text-3xl font-bold text-pink-400">{connections.length}</p><p className="text-xs text-[#4A6485]">Conexiones</p></div>
 <div className="p-4 bg-white/60 rounded-xl"><p className="text-3xl font-bold text-[#1E86E5]">{avgRating.toFixed(1)}</p><p className="text-xs text-[#4A6485]">Calificación promedio</p></div>
 </div>

 <div className="space-y-2">
 <h3 className="text-lg font-semibold text-[#0B2240]">Tus conexiones</h3>
 ...
 <button onClick={() => window.location.reload()} className="px-6 py-3 bg-pink-600 hover:bg-pink-500 text-[#0B2240] rounded-xl font-semibold transition-colors">Jugar de Nuevo</button>
 ...
 <div>
 <p className="text-sm text-[#4A6485]">Ronda {currentRound + 1} de {TOTAL_ROUNDS}</p>
 <div className="w-32 h-2 bg-white/50 rounded-full mt-1 overflow-hidden"><motion.div className="h-full bg-pink-500" initial={{ width: 0 }} animate={{ width: `${((currentRound + 1) / TOTAL_ROUNDS) * 100}%` }} /></div>
 </div>
 </div>
 <div className="flex items-center gap-4">
 <div className={`flex items-center gap-2 px-4 py-2 rounded-xl font-mono text-lg font-bold ${timeRemaining < 30 ? 'bg-red-500/20 text-red-400' : 'bg-white/50 text-[#0B2240]'}`}><Clock className="w-4 h-4" />{formatTime(timeRemaining)}</div>
 <button onClick={onClose} className="p-2 hover:bg-[rgba(46,150,245,0.08)]0 rounded-xl"><X className="w-5 h-5 text-[#4A6485]" /></button>
 </div>
 </div>

 <div className="flex-1 flex overflow-hidden">
 <div className="w-72 border-r border-[rgba(46,150,245,0.14)] bg-white/60 p-6">
 ...
 <div><h3 className="text-xl font-bold text-[#0B2240]">{currentPartner.name}</h3><p className="text-pink-400">{currentPartner.role}</p><p className="text-sm text-[#4A6485]">{currentPartner.department}</p></div>
 </div>
 <div className="mt-8">
 <p className="text-xs text-[#4A6485] uppercase tracking-wider mb-2">Tema sugerido</p>
 ...
 <button onClick={handleRoundEnd} className="w-full mt-6 py-3 border border-[rgba(46,150,245,0.16)] hover:border-[rgba(46,150,245,0.16)] text-[#4A6485] hover:text-[#0B2240] rounded-xl text-sm transition-colors">Saltar conversación</button>
 </div>
 ...
 <div className={`max-w-[70%] px-4 py-3 rounded-2xl ${msg.sender === 'Tú' ? 'bg-pink-600 text-[#0B2240] rounded-br-md' : 'bg-white/50 text-[#1B3A5C] rounded-bl-md'}`}><p>{msg.message}</p></div>
 </div>
 ))}
 </div>
 <div className="p-4 border-t border-[rgba(46,150,245,0.14)]">
 <div className="flex gap-2">
 <input type="text" value={chatInput} onChange={(e) => setChatInput(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && handleSendMessage()} placeholder="Escribe un mensaje..." className="flex-1 px-4 py-3 bg-white/50 border border-[rgba(46,150,245,0.16)] rounded-xl text-[#0B2240] placeholder-[#9CB0CA] focus:outline-none focus:border-pink-500" />
 <button onClick={handleSendMessage} className="px-4 py-3 bg-pink-600 hover:bg-pink-500 text-[#0B2240] rounded-xl transition-colors"><Send className="w-5 h-5" /></button>
 </div>
 </div>
 </div>
 </div>
 ...
 <AnimatePresence>
 {showRating && (
 <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-[#0B2240]/35 backdrop-blur-[10px] flex items-center justify-center z-50">
 <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="bg-white/60 rounded-2xl p-8 max-w-md w-full mx-4 border border-[rgba(46,150,245,0.16)] text-center">
 <p className="text-[#4A6485] mb-2">¿Cómo fue tu conversación con</p>
 <h3 className="text-2xl font-bold text-[#0B2240] mb-6">{currentPartner.name}?</h3>
 <div className="flex justify-center gap-2 mb-8">
 {[1, 2, 3, 4, 5].map((star) => (
 <motion.button key={star} whileHover={{ scale: 1.2 }} whileTap={{ scale: 0.9 }} onClick={() => handleRateConversation(star)} className="text-4xl text-[#6B83A0] hover:text-[#1E86E5] transition-colors">★</motion.button>
 ))}
 </div>
 <p className="text-sm text-[#4A6485]">4-5 estrellas = Conexión guardada</p>
 </motion.div>
 </motion.div>
 )}
 </AnimatePresence>
