...
export const FLOOR_SPECS: Record<FloorType, FloorSpec> = {
  // ── BASKETWEAVE (parquet de bloques perpendiculares) ─────────────────────
  // Reemplaza las maderas horizontales (que aliasaban moiré direccional)
  // por un parquet basketweave estilo Versailles-lite. Cada bloque de
  // `tileSize × tileSize` contiene 2 planks; bloques adyacentes alternan
  // orientación. Sin líneas horizontales largas → bye moiré direccional.
  // tileSize 0.45m → bloque de 45cm con planks de 45×22cm. Similar al
  // tamaño aparente de los planks horizontales previos (0.7 × 0.175 cm).
  [FloorType.WOOD_OAK]: {
    pattern: 'BASKETWEAVE',
